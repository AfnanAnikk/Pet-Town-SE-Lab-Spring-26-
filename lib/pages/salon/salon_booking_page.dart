...
import 'salon_booking_sheets.dart';
import 'salon_booking_success_page.dart';
...
class SalonBookingPage extends StatefulWidget {
  ...
  const SalonBookingPage({super.key, required this.salon, this.selectedSlot});
...
class _SalonBookingPageState extends State<SalonBookingPage> {
  ...
  void _handleBooking() async {
    if (petDetails == null || selectedService == null || reasonForVisit == null || paymentMethod == null || selectedDate == null || selectedTime == null) {
    ...
    final result = await ApiService.createSalonBooking(bookingData);
    ...
    if (result['success']) {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => SalonBookingSuccessPage(
            salon: widget.salon,
  ...
  void initState() {
    ...
    for (final slot in widget.salon.availableSlots) {
  ...
  void _showServiceSheet() async {
    final result = await showModalBottomSheet<String>(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => ServiceSheet(initialService: selectedService),
    );
    if (result != null) {
      setState(() {
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildField(
                    ...
                    title: 'Service',
                    value: selectedService ?? 'Tap to select',
                    isRequired: selectedService == null,
                    onTap: _showServiceSheet,
                  ),
                  
                  _buildField(
                    icon: Icons.folder_open_outlined,
                    title: 'Instructions for Groomer',
                        ...
                        if (_discountAmount > 0) ...[
                          Row(
                            children: [
                              ...
                              Text(
                                'BDT ${widget.salon.price - _discountAmount}',
                                style: const TextStyle(
                                  fontSize: 18,
                                  fontWeight: FontWeight.bold,
                                  color: Colors.green,
  ...
  Future<void> _loadAvailableVouchers() async {
    ...
    // Assuming salon id can be fetched this way from backend endpoint. It's stored as UUID usually.
    final ownerId = "placeholder"; // We can fetch vouchers by salonId if implemented in backend, but we'll adapt endpoint
    final res = await ApiService.getSalonVouchers(widget.salon.id); 
    if (res['success']) {
      setState(() {
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          ...
                          final res = await ApiService.validateSalonVoucher(code, widget.salon.id);
                          if (res['success'] && res['data']?['valid'] == true) {
                            ...
                            setState(() {
                              ...
                              _discountAmount = ((widget.salon.price * _discountPercent) / 100).round();
                            });
                            if (mounted) {
